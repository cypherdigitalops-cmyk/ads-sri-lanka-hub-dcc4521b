...
const TITLE = "Advertising in Negombo — Complete Guide to Channels, Rates & Agencies";
const DESC = "Plan a winning campaign in Negombo, Sri Lanka. We cover billboards, radio, newspapers, TV, digital, BTL and activations in Negombo — with local rates, top locations and free guidance.";
const CITY = "Negombo";
const SLUG = "advertising-in-negombo";
...
const LOCAL_BUSINESS_JSONLD = {
  ...
  geo: { "@type": "GeoCoordinates", latitude: 7.2083, longitude: 79.8358 },
};
...
const FAQS = [
  { q: `How much does advertising cost in ${CITY}?`, a: `Costs in ${CITY} range from Rs 5,000 for a small flyer print run to Rs 500,000+/month for premium billboards. Digital campaigns start around Rs 25,000/month. We give free, no-obligation quotes for every channel.` },
  { q: `Which channel works best in ${CITY}?`, a: `It depends on your audience. Billboards and radio dominate for mass reach in ${CITY}; digital and social media win for targeted B2C; activations and BTL work well in malls and high-footfall junctions.` },
  { q: `Can you book billboards in ${CITY}?`, a: `Yes. We have access to prime hoarding locations across ${CITY} including Negombo Main Street, Lewis Place, Ethukala beachfront — and we handle permits, printing and installation end-to-end.` },
...
export const Route = createFileRoute(`/${SLUG}` as "/advertising-in-negombo")({
  head: () => ({
    meta: [
      { title: TITLE },
  ...
  return (
    <PageShell>
      ...
      <section className="mx-auto max-w-3xl px-4 py-12 text-muted-foreground">
        ...
        <p className="mt-4">{CITY} is one of Sri Lanka's most important advertising markets. Whether you're running a national brand campaign, a regional retail push or a hyper-local activation, our team in {CITY} handles strategy, media buying, creative production and on-ground execution end-to-end.</p>
        <h3 className="mt-8 text-xl font-semibold text-foreground">Local media we work with</h3>
        <p className="mt-3">Yes FM coastal, Sirasa FM, Lakhanda, regional pages of Lankadeepa and Daily News, hotel-focused tourism press.</p>
        <h3 className="mt-8 text-xl font-semibold text-foreground">Prime billboard & OOH locations in {CITY}</h3>
        <p className="mt-3 flex items-start gap-2"><MapPin className="mt-1 h-4 w-4 flex-none text-accent" /> Negombo Main Street, Lewis Place, Ethukala beachfront, Kochchikade junction, Katunayake Expressway interchange.</p>
        ...
        <ul className="mt-3 space-y-2">
          ...
          <li className="flex gap-2"><CheckCircle2 className="mt-1 h-4 w-4 flex-none text-accent" /> BTL activations, mall promotions, sampling and roadshows</li>
